...
sound.Add( {
	...
	level = 100,
	pitch = 100,
	sound = "npc/turret_floor/alarm.wav"
} )


function ENT:Initialize()
	self:SetModel("models/misfitworks/printersmall.mdl")
...
function ENT:Think()
	...
	if self:GetNW2Float("Battery") > 0 and CurTime() > (self.timer + self.PrintTime) then
		...
		if self.curStored == self.MaxStorage then
			self:SetNW2Int("MoneyStored", self.curStored)
		end
...
function ENT:Use(a, c, usetype, value)
	...
	and HitPosition.x <= 8.450662612915
	and HitPosition.y >= -9.0903434753418
	and HitPosition.y <= -1.5687268972397
	...
	if TraceLine.Entity == self
	and HitPosition.x >= 1.1818530797958
	and HitPosition.x <= 8.910196352005
	and HitPosition.y >= -2.0663061141968
	and HitPosition.y <= 2.8833808898926
	and c:KeyDown(IN_USE) then
		self:CollectPrinter(c)
	end

	if TraceLine.Entity == self
	and HitPosition.x >= 5.0511174201965
	and HitPosition.x <= 8.3780012130737
	and HitPosition.y >= -1.3088371753693
	and HitPosition.y <= 1.9363125562668
	and c:KeyDown(IN_USE) then
	self:EmitSound("sosalarm")
	timer.Simple( 3, function()
		self:StopSound("sosalarm")
		self:Explode()
	end)
end
	...
	if TraceLine.Entity == self
	and HitPosition.x >= 5.0222773551941
	and HitPosition.x <= 8.316725730896
	and HitPosition.y >= 2.2390611171722
	and HitPosition.y <= 9.7503366470337

	and c:KeyDown(IN_USE) and self:GetNW2Float("Temperature") > 16 then
		self:SetNW2Float("Temperature", self:GetNW2Float("Temperature") - 3)
		c:addMoney(-self.CoolPrice)
